...
if [ ! -f "./bin/arduino-cli" ]; then
    curl -fsSL https://raw.githubusercontent.com/arduino/arduino-cli/master/install.sh -o install.sh
    chmod +x install.sh
    ./install.sh
fi
...
# Add optimization flags for Anemoia
ANEMOIA_BUILD_FLAGS="$BUILD_FLAGS -DOPTIMIZATION_FLAGS -O3"
...
./bin/arduino-cli compile --fqbn esp32:esp32:esp32 --build-property "build.extra_flags=$ANEMOIA_BUILD_FLAGS" --output-dir build Anemoia-ESP32
